...
// Manejar limpieza de token cuando se cierre la pestaña
if (isset($_GET['logout_beacon'])) {
    $user_id = isset($_POST['user_id']) ? intval($_POST['user_id']) : 0;
    if ($user_id > 0) {
        $conexion->query("UPDATE usuarios SET session_token = NULL WHERE id = $user_id");
    }
    exit();
}
...
<head>
    ...
    <script>
    // Función para limpiar el token al salir
    function handleTabClose() {
        const data = new URLSearchParams();
        data.append('user_id', '<?php echo $_SESSION['usuario_id']; ?>');
        navigator.sendBeacon('index.php?logout_beacon=1', data);
    }

    // Función para cierre de sesión manual
    function cerrarSesion() {
        const data = new URLSearchParams();
        data.append('user_id', '<?php echo $_SESSION['usuario_id']; ?>');
        
        fetch('index.php?logout_beacon=1', {
            method: 'POST',
            body: data
        }).then(() => {
    ...
    // Respaldos para diferentes navegadores
