...
// --- Manejar la limpieza del token si es una petición AJAX/Fetch ---
if (isset($_GET['clean_token'])) {
    if (!empty($_POST['user_id'])) {
        $user_id = intval($_POST['user_id']);
        $sql = "UPDATE usuarios SET session_token = NULL WHERE id = ?";
...
// --- Verificación normal de sesión (lógica original) ---
...
<head>
    ...
    <script>
    // Función mejorada para limpiar el token
    function limpiarTokenAlSalir() {
        const url = window.location.href.split('?')[0] + '?clean_token=1';
        const data = new URLSearchParams();
        data.append('user_id', '<?php echo $_SESSION['usuario_id']; ?>');

        // Opción 1: sendBeacon (óptimo para cierre de pestaña)
        if (!navigator.sendBeacon(url, data)) {
            // Opción 2: fetch con keepalive (respaldo)
            fetch(url, {
                method: 'POST',
                body: data,
                keepalive: true
    ...
    // Eventos para cierre de pestaña
    window.addEventListener('beforeunload', function(e) {
        if (!e.target.activeElement?.href?.startsWith(window.location.origin)) {
            limpiarTokenAlSalir();
        }
    });

    window.addEventListener('unload', limpiarTokenAlSalir); // Respaldo
    </script>
</head>
            ...
            <div class="flex justify-between items-center h-16 md:h-20">
                ...
                <div class="flex items-center space-x-3 md:space-x-4">
                    ...
                    <a href="#" onclick="cerrarSesion(); return false;" 
                       class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-md text-sm font-medium transition-colors duration-200 flex items-center justify-center h-8 md:h-9 whitespace-nowrap">
            ...
            <div class="bg-white p-8 rounded-lg shadow-md">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white p-4 rounded-lg border border-gray-200">
                        ...
                        <a href="https://wa.me/[TU_NUMERO]?text=Hola,%20necesito%20ayuda" target="_blank" class="inline-flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white text-sm font-normal py-2 px-3 rounded transition-colors duration-200">
